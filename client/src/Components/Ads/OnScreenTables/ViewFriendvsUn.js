...
class ViewFriendVsUn extends Component {
  /* eslint-disable */
  constructor(props) {
    super(props);
    this.state = {
  ...
  buildSpenderSummary() {
    ...
    for(var m = 0; m < this.state.markets.length; m++){
      ...
      if(m === 0) {
        tabs.push(<div id={this.state.markets[m]} className="tabcontent firsttab"><h3>{this.state.markets[m]}</h3><table><tbody>{topTable}{middleTable}{botttomTable}</tbody></table></div>)
      } else {
        tabs.push(<div id={this.state.markets[m]} className="tabcontent"><h3>{this.state.markets[m]}</h3><table><tbody>{topTable}{middleTable}{botttomTable}</tbody></table></div>)
      }
