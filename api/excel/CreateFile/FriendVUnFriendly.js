...
async function makefvuf(req,res, spenderResults, earliestStartEndBuys, markets, mediatypes, spenders, supports){
...
var columnhead = wb.createStyle({
  ...
  ws.cell(1, 1,1,8,true).string('Friendly vs Unfriendly').style(style1);
...
for(var m = 0; m < markets.length; m++){
      ...
      for(var a = 0; a < bottomMediaTypes.length; a++){
        ...
        if(a == bottomMediaTypes.length - 1) {
            ws.cell(
              6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + a - bottomMediaTypes.length + 1,
              1,
        ...
        ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + a, 4).string(bottomMediaTypes[a]).style(emptyCell);
        ...
        var overallTrps = 0;
            ...
            for(var o = 0; o < times.length; o++) {
              ...
              for(var i = 0; i < spenderResults.length; i++) {
                    ...
                    ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + a, 4 + o + (o + 1)).number(Math.round(broadcastTotal)).style(cellStyle);

                    if(bottomMediaTypes[a] == "Total"){
                        ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + a, 5 + o + (o + 1)).style(blankStyle);
                    } else {
                        ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + a, 5 + o + (o + 1)).number(Math.round(trpsTotal)).style(cellStyle);
                    }
      ...
      if(customStartRange > 0){
        ...
        if(bottomMediaTypes[a] == "Total"){
          ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + a, 4 + (times.length * 2) + 2 - (customStartRange * 2 - 2)).style(blankStyle);
        } else {
        ...
        ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + a
        , 4 + (times.length * 2) + 1).number(Math.round(overallBroadcast)).style(totalCell);

        if(bottomMediaTypes[a] == "Total"){
          ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + a, 4 + (times.length * 2) + 2).style(blankStyle);
        } else {
      ...
      for(var v = 0; v < varianceTypes.length; v++) {
        ...
        //Media Type cell
       ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + bottomMediaTypes.length + 1 + v, 2, 6 +
       (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + bottomMediaTypes.length + 1 + v, 3, true).string(varianceTypes[v] + " Variance").style(emptyCell)
        //Blank cell
        ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + bottomMediaTypes.length + 1 + v, 4).string("").style(emptyCell);
          ...
          if(req.body.start > times.length - o && req.body.start > 1 && o != times.length) {
            ...
            for(var i = 0; i < spenderResults.length; i++) {
                    ...
                    var secondTime = moment(times[o].substr(7,9), "MM/DD").add(1, "days")
                      ...
                      ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + bottomMediaTypes.length + 1 + v, 4 + o + (o + 1)).number(Math.round(amountTotal)).style(cellStyle);

                      if(varianceTypes[v] == "Total"){
                        ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + bottomMediaTypes.length + 1 + v, 4 + o + (o + 2)).style(blankStyle);
                      } else {
                        ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + bottomMediaTypes.length + 1 + v, 4 + o + (o + 2)).number(Math.round(trpsTotal)).style(cellStyle);
                      }
            ...
            if(customStartRange > 0){
              ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6))  + bottomMediaTypes.length + 1 + v, 4 + (times.length * 2) + 1 - (customStartRange * 2 - 2)).number(Math.round(overAllAmount)).style(totalCell);

              if(varianceTypes[v] == "Total"){
                ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6)) + bottomMediaTypes.length + 1 + v, 4 + (times.length * 2) + 2 - (customStartRange * 2 - 2)).style(blankStyle);
              } else {
                ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6))  + bottomMediaTypes.length + 1 + v, 4 + (times.length * 2) + 2 - (customStartRange * 2 - 2)).number(Math.round(overallTrps)).style(totalCell);
              }


            } else {
              ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6))  + bottomMediaTypes.length + 1 + v, 4 + (times.length * 2) + 1).number(Math.round(overAllAmount)).style(totalCell);

              if(varianceTypes[v] == "Total"){
                ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6))  + bottomMediaTypes.length + 1 + v, 4 + (times.length * 2) + 2).style(blankStyle);
              } else {
                ws.cell(6 + (m * (mediatypes.length + bottomMediaTypes.length + varianceTypes.length + 6))  + bottomMediaTypes.length + 1 + v, 4 + (times.length * 2) + 2).number(Math.round(overallTrps)).style(totalCell);
              }
